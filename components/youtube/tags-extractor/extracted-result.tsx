...
import { Copy, Hash, Check, Sparkles, X } from "lucide-react";
...
const ExtractedResult = ({ data }: ExtractedResultProps) => {
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-8">
        ...
        <div className="flex gap-3 w-full md:w-auto">
          <Button
            ...
            size="sm"
            onClick={toggleHash}
            className={`flex-1 md:flex-none flex items-center gap-2 rounded-xl px-4 py-5 transition-all duration-300 font-semibold border-zinc-300  ${
              isHashed ? " text-primary" : ""
            }`}
          >
          ...
          <Button
            ...
            size="sm"
            onClick={handleCopy}
            className="flex-1 md:flex-none flex items-center gap-2 rounded-xl px-5 py-5 shadow-md shadow-primary/20 hover:shadow-primary/40 transition-all duration-300 font-semibold"
          >
      ...
      <div className="flex flex-wrap gap-2 p-6 bg-muted/20 rounded-2xl border border-muted-foreground/5 min-h-[150px]">
        <AnimatePresence>
          {tags.map((tag, index) => (
            ...
            >
              <span>{isHashed && !tag.startsWith("#") ? `#${tag}` : tag}</span>
              <button
                onClick={() => removeTag(index)}
                className="p-0.5 hover:bg-zinc-800 rounded-md transition-colors text-zinc-500 hover:text-zinc-300"
              >
                <X className="w-3 h-3" />
              </button>
            </motion.div>
...
export default ExtractedResult;
